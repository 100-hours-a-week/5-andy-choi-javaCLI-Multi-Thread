...
//        }
        ...
        Map<String, Long> quant = new HashMap<>();
        for (String key : product_lst.keySet()){
            quant.put(key,product_lst.get(key).getQuantity());
        }
        ...
        for(int i =0;i < numOfUsers;i++){
            User user = new User(new Account(),"Andy"+i,"1234");
            user.getAccount().deposit(100_000_000_000L);
        ...
        compare(product_lst,quant,numOfUsers*5l);
    }

    public static void compare(Map<String,Product> map, Map<String,Long> quant,Long offset){
        for(String key : map.keySet()){
            Long quantity = map.get(key).getQuantity();
            if(!quantity.equals(quant.get(key) - offset)){
                System.out.println("================= Race Condition 발생! =================");
                System.out.println("주식명:"+key);
                System.out.println("기대 수량:"+(quant.get(key)-offset));
                System.out.println("현재 수량:"+map.get(key).getQuantity());
            }
        }
    }
}